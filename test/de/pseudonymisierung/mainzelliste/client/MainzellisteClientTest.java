...
public class MainzellisteClientTest {
	...
	@Test
	public void test() throws MainzellisteNetworkException,
		...
		for (String tempId : s.getTempIds()) {
			...
			assertEquals("Got different id for same temp id", ids.get(tempId),
					id);
			assertEquals(
					"Got different id for same temp id in restored session",
					id, s2.getId(tempId));
